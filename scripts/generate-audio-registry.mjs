...
function groupKeyForFile(folderName, fileName) {
  ...
  return `${folderName}/${prefix}`.replace(/\\/g, '/');
}
...
async function main() {
  ...
  for (const fullPath of allFiles) {
    ...
    const folderKey = folder === '.' ? path.basename(fileName, ext) : folder;
    const key = groupKeyForFile(folderKey, fileName);
  ...
  for (const [key, files] of [...groups.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    const groupFolder = key.includes('/') ? key.slice(0, key.lastIndexOf('/')) : '';
    const chosen = pickPreferredFormat(files)
      .map((full) => path.basename(full))
      .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
    outputGroups[key] = {
      basePath: groupFolder ? `audio/sfx/${groupFolder}` : 'audio/sfx',
      files: chosen,
    };
  }
