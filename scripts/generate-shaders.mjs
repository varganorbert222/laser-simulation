 ...
 * slots.ts MAX_GPU_LIGHTS / MAX_GPU_MEDIA / MAX_GPU_FOGS, writes src/generated/shaders/*.{glsl,ts}.
 */
...
function readSlots() {
  ...
  const fogs = text.match(/export const MAX_GPU_FOGS\s*=\s*(\d+)/);
  if (!lights || !media || !fogs) {
    throw new Error('Could not parse MAX_GPU_LIGHTS / MAX_GPU_MEDIA / MAX_GPU_FOGS from slots.ts');
  }
  return {
    lights: Number(lights[1]),
    media: Number(media[1]),
    fluids: Number(fogs[1]),
  };
}
...
/** Sample density from 2D slice-atlas (CPU twin: engine/physics/fog/atlas.ts). */
...
function writeOut(baseName, glsl, exportName) {
  ...
  const ts = `/** Auto-generated by scripts/generate-shaders.mjs â€” do not edit. */\nexport const ${exportName} = ${JSON.stringify(glsl)};\n`;
...
function main() {
  ...
  console.log(`generate-shaders: MAX_GPU_LIGHTS=${lights} MAX_GPU_MEDIA=${media} MAX_GPU_FOGS=${fluids}`);
  ...
  const indexTs = `/** Auto-generated by scripts/generate-shaders.mjs â€” do not edit. */
...
`;
  ...
  console.log('done â†’', path.relative(root, outDir));
}
