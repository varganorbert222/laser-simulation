...
export async function loadSfxRegistry(configBaseUrl: string): Promise<SfxRegistry | null> {
  try {
    const url = `${configBaseUrl.replace(/\/+$/, '')}/sfx/registry.json`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return (await res.json()) as SfxRegistry;
  } catch (err) {
    console.warn('[Audio] sfx registry load failed — registry refs will no-op', err);
    return null;
  }
}
