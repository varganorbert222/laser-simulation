...
| Path | Purpose |
|------|---------|
| `manifest.json` | Models (fixtures/props/stage) + skyboxes — paths relative to `/assets` |
...
Binary media (GLB, WAV, PNG, …) live in [`assets/`](../assets/). Drop files there, then wire ids in these JSON catalogs.

Editor catalogs that are not media libraries (`quality`, fixtures labels, demo scenes) stay under `public/catalog/`.
